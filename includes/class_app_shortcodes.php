...
class App_Shortcode_Services extends App_Shortcode {
	public function __construct () {
		$this->_defaults = array(
			'select' => array(
				'value' => __('Please select a tour:', 'appointments'),
				'help' => __('Text above the select menu. Default: "Please select a service"', 'appointments'),
				'example' => __('Please select a tour:', 'appointments'),
			),
			'show' => array(
	...
	public function process_shortcode ($args=array(), $content='') {
		...
		$s .= '</select>';
		$s .= '<div class="appointments-field appointments-attendeesnumberonly-field"><label for="appointments-attendeesnumberonlyc0dd276d7b88a1e0d4fe71c58c963f5f" style="display: block;margin-top: 10px;margin-bottom: 10px;"><span>Attendees ( number only) </span></label><input type="text" id="appointments-attendeesnumberonlyc0dd276d7b88a1e0d4fe71c58c963f5f" class="appointments-field-entry appointments-attendeesnumberonly" name="attendeesnumberonly" value="'.$_REQUEST['attendees'].'"></div>';		
		$s .= '<input type="button" class="app_services_button" style="display: block;margin-top: 10px;margin-bottom: 10px;" value="'.$show.'">';
		...
		$href2 = add_query_arg( array( "wcalendar"=>false, "app_num" => false, "app_num" => false ) );
		$href2 = apply_filters( 'app_num_href', add_query_arg( array( "wcalendar"=>$wcalendar, "app_num" => "__app_num__" ), $href ) );
		...
		$script .= "var attendees_num=$('.appointments-attendeesnumberonly').val();";
		...
		if ( $autorefresh ) {
		$script .= "window.location.href='".$href."'.replace(/__selected_service__/, selected_service)+'&attendees='+attendees_num+''";
		}
		$script .= "});";
...
/*		$ret .= '<div class="appointments-phone-field">';
		$ret .= '<label><span>Attendees ( number only)</span><input type="number" class="appointments-phone-field-entry" id="' . esc_attr(apply_filters('app-shortcode-confirmation-att_field_id', 'appointments-field-customer_phone')) . '" value="'.$p.'" /></label>';
		$ret .= '</div>';
*/
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
		...
		$ret .='<div class="appointments-field appointments-coupon-field"><label for="appointments-attendeesnumberonlyc0dd276d7b88a1e0d4fe71c58c963f5f"><span>Coupon Code</span></label><input type="text" id="cupon" class="appointments-field-entry appointments-coupon-field-entry" name="coupon" value=""></div>';

		$ret .= '<div style="clear:both"></div>';
		
		$ret .='<div class="appointments-field appointments-price-main-field"><label for="appointments-attendeesnumberonlyc0dd276d7b88a1e0d4fe71c58c963f5f"><span>Payment Option</span></label><input type="radio" id="cupon" class="appointments-field-entry appointments-refund-field-entry" name="refund_policy" value="twenty" checked> Non-refundable deposit($20) <input type="radio" id="cupon" class="appointments-field-entry appointments-refund-field-entry" name="refund_policy" value="full"> Full price</div>';




		$ret .= '<div class="appointments-confirmation-price-new" style="display:none">';
		$ret .= '</div>';

		$ret .= '<div class="appointments-confirmation-price-coupon" style="display:none">';
		$ret .= '</div>';
		...
		if ( is_user_logged_in() || 'yes' != $appointments->options["login_required"] ) {
			...
			$script .= 'var app_value_att = $(this).find(".appointments_take_appointment").attr("attendent");';
			$script .= '$(".appointments-confirmation-final-value").val(app_value);';
			$script .= 'var pre_data = {action: "pre_confirmation", value: app_value, app_value_att: app_value_att, nonce: "'. wp_create_nonce() .'"};';
			$script .= '$.post(_appointments_data.ajax_url, pre_data, function(response) {
						...
						if ( response && response.error )
							...
							var price_another = response.price ;
							$(".appointments-confirmation-price").html(price_another);
							if (response.price != "0"){
		...
		$script .= '$.post(_appointments_data.ajax_url, post_data, function(response) {
						...
						if ( response && response.error ) {
							...
							var coupon_price = response.coupon;
							if(coupon_price!=0 && coupon_price!=null){
							$(".appointments-confirmation-price-coupon").html("Coupon Price: $"+coupon_price+"");
							$(".appointments-confirmation-price-coupon").show();
							}else{
							$(".appointments-confirmation-price-coupon").hide();
								}
							
							$(".appointments-paypal").find(".app_amount").val(new_price);
							$(".price_changer").html(new_price+" USD");
							$(".appointments-paypal").find(".app_custom").val(response.result);
							var new_bat_val = "Please confirm $" + new_price + " USD payment for Default Service";
							$(".appointments-confirmation-price-new").html("Total Price: $"+new_price+"");
							if(new_price !=0){
								$(".appointments-confirmation-price-new").show();
								}
							
							$(".appointments-paypal").find(".app_submit_btn").val(new_bat_val);
