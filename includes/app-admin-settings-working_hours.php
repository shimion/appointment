...
add_action( 'admin_menu', 'working_calender_mine_declear' );

/** Step 1. */
function working_calender_mine_declear() {
	add_menu_page( 'Working Calendar', 'Working Calendar', 'read', 'working-calender', 'working_calender_mine' );
}
function working_calender_mine(){
...
<?php _e( '<h2>Your Schedule Calendar</h2>', 'appointments'); ?>
<br />
<br />
<?php
				...
				<script>
					jQuery(document).ready(function($) {
						...
						//	$(".eventCalendar-subtitle").attr("date", date_each);
								var three_data = {action: "check_user", date: date_each, nonce: "<?php wp_create_nonce() ?>"};	
