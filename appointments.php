...
class Appointments {
	...
	function delete_user_app(){
		...
		global $wpdb;
		$user_ID = get_current_user_id();
		$result = $wpdb->delete( 'wp_three_line_appointment', 							
							array( 
								'worker' => $user_ID, 
								'date' => $date, 
							),
...
function get_worker_three_line($ccs, $cce){
		...
		if(!empty($working_hours) and is_array($working_hours)){
			...
			if(strtotime ($worker_hour->working_hours_start) <= $hour_min){
					if(strtotime ($worker_hour->break_enable)=='0'){
		...
		if ((isset($_POST['payment_status']) || isset($_POST['txn_type'])) && isset($_POST['custom'])) {
			...
			switch ($_POST['payment_status']) {
				...
				case 'Processed':
					...
					$message = sprintf( __('You have an appointment at %s', 'appointments'), $this->get_start_date_for_sending_email($_POST['custom']) );
